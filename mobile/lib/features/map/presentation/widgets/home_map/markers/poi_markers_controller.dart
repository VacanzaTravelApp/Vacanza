...
class PoiMarkersController {
  PoiMarkersController(this._map, {this.onPoiTap});
  ...
  Future<Uint8List> _bitmapForCategory(String category) async {
    ...
    try {
      final bytes = await PoiMarkerBitmapBuilder.bitmapForRawCategory(category);
      _iconCache[key] = bytes;
      log(
        '[PoiMarkersController] built icon for catalog key "$key" (${bytes.length} bytes)',
      );
      return bytes;
    } catch (e, st) {
